...
terminated. Multiple ENI on single EC2 is possible. When a ENI is moved from
one instance to another, the traffic is redirected.

> ENI connects resources to VPC
...
> SSL/HTTPS termination speeds up the decryption process at ELB to reduce burden
> on EC2
...
When targets are specified using instance ID, traffic is routed to instance
using primary private IP address specified in the primary network interface of
the instance. Alternatively, target can be specified using any private IP (i.e.
...
- private hosted zone
  ...
  - how R53 responds to DNS queries
  - need to enabled DNS hostnames and DNS rsolution for private hosted zones
...
- custom HTTP
  ...
  - custom origin i.e. customer on-prem servers

> cannot point to R53 or any DNS record; doesnt belong in any VPC
...
unhealthy. Alternatively the endpoint group can be used for blue-green
deployment by modifying the weights to be directed to.
...
> does not necessarily handles spike in load better, user CloudFront instead
...
peered VPC can be referenced between same region cross account. Traffic between
two VPC never traverses public internet.
...
- interface endpoints (powered by PrivateLink/VIF)
...
![vpc endpoints](../static/vpc-endpoints.PNG)
...
share cross account using Resource Access Manager (RAM). Route Table limits
which VPC can communicate to another. It works with DX, VPN connection and
VPCs. It is the only service AWS provides that supports IP multicast. With
Transit Gateway it is possible to create a VPC that host "shared services" for
all the components in the network topology to use with network isolation. It
helps to reduce administrative overhead and cost.

> Transit Gateway is more expensive than setting RAM and shares subnet; it is 
> charged based on number of connection per hour per GB
...
![flow log syntax](../static/flow-log-syntax.PNG)
...
> need work, correct config of IPSec VPN connection VGW on AWS and a customer
> gateway on on prem side of vpn?

![s2s setup](../static/s2s-setup.PNG)
...
[cloud hub](../static/cloud-hub.PNG)
...
v6. Cost for data transfer is lower than over internet.
...
![dx resilient](../static/dx-resilient.PNG)
...
![ipv6 route](../static/ipv6-route.PNG)

## Networking Cost in AWS

> its simplified

![ec2 nw cost](../static/ec2-nw-cost.PNG)
...
![s3 nw cost](../static/s3-nw-cost.PNG)
...
![nat vpc gw](../static/nat-vpc-gw.PNG)
