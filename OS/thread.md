...
### strategy to avoid race condition and achieve thread safety
...
1. Mutex: access to shared data is serialized using mechanism to ensure only one thread read or write the data at any time. having side effects inlcuding deadlocks, livelocks and resource starvation
2. atomic operations: shread data is accessed by using atomic operations which cannot be interupted by other threads. it forms basis of many thread locking mechanism and are used to implement mutex primitives.
...
- prevention - preventing one of the four coffman conditions
  ...
  - circular wait avoidance is achieved by disabling interupts during critical sections and using hierarchy to determine partial ordering of resources. if no hierarchy exists, memory address of resource can be used to determine ordering and resources are requested in the increasing order of enumeration. dijkstra's solution can also be used.

### race condition

a condition where the system substansive behavior is dependent on the sequence or timing of other **uncontrollable** events, or in the case of threads (processes) its the sequence or timing of threads (processes). its difficult to reproduce as the result is non-deterministic.
