...
the model partition the flow of data in a communication into seven abstraciton layers to describe network communication from physical bits transfer to highest level representation of data. each intermediate layer serves a well-defined class of functionality to the layer above and below it. there are two major components on the conceptual model, the abstract model of networking and a set of specific protocols. it standardized network concepts and stops the protocol war.

communication protocol enables an entity in one host to interact with other entity at the **same** layer in another host. at each level, two entities at the communicating devices exchanges protocol data units (PDU) by means of layer N protocol. each PDU contains a payload or service data unit along with the protocol related headers or footers. from bottom up, each layer abstracts complexities through the services on implementation. same layer can do a peer-to-peer communication.
...
|-------|-----|----------|--------| 
| application | data | high level protocol for resource sharing or remote file access eg. HTTP | provides function including file sharing, message handling and database through common protocol eg HTTP, FTP, SMTP, DHCP etc. |
